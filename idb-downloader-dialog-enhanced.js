...
@keyframes greenWaveExpansion {
  ...
  25% {
    ...
    box-shadow: 
      0 0 0 10px rgba(76, 175, 80, 0.6),
      0 0 0 12px rgba(255, 255, 255, 0.4);
  }
  50% {
    transform: scale(1.1);
    box-shadow: 
      0 0 0 20px rgba(76, 175, 80, 0.4),
      0 0 0 24px rgba(255, 255, 255, 0.3);
  }
  75% {
    transform: scale(1.05);
    box-shadow: 
      0 0 0 30px rgba(76, 175, 80, 0.2),
...
@keyframes tickGlow {
  ...
  50% { 
    box-shadow: 
      ...
      0 10px 35px rgba(76,175,80,0.25),
...
/* ENHANCED: Dashed outline with light blue background */
.glassy-btn{
...
/* ENHANCED: Expanded click area for better touch support */
.glassy-btn::before {
  content: '';
  position: absolute;
  top: -4px;
...
/* ENHANCED: Improved completion tick with better background and more solid/curved tick */
.completion-tick{
  display: flex;
  flex-direction: column;
  align-items: center;
...
.tick-container{
  ...
  align-items: center;
  justify-content: center;
  background: linear-gradient(135deg, #81c784 0%, #66bb6a 20%, #4caf50 40%, #43a047 60%, #388e3c 80%, #2e7d32 100%);
  ...
  animation: 
    ...
    greenWaveExpansion 2s ease-out 1600ms infinite,
    outlineGlow 2.5s ease-in-out infinite 1800ms;
  position: relative;
  box-shadow: 
...
.tick-mark{
  ...
  animation: completionTick 900ms cubic-bezier(0.68, -0.55, 0.265, 1.55) 700ms both;
  position: relative;
  filter: drop-shadow(0 3px 6px rgba(0,0,0,0.15));
...
`;
  ...
  setTimeout(async () => {
    try {
      ...
      let actualDownloadStarted = false; // ENHANCED: Track when actual download starts

      // ENHANCED: Fixed progress bar management
      const setProgressIndeterminate = (indeterminate) => {
        try {
          ...
          if (indeterminate) {
            ...
            // Switch to determinate when we have actual progress
      ...
      // ENHANCED: Fixed browser button state management
      ...
      // ENHANCED: Progress metrics with fixed determinate switching
      const updateProgressMetrics = (received, total, percent, speed) => {
        try {
          ...
          // FIXED: Switch to determinate when actual download starts (not just when progress > 0)
        ...
        try {
          ...
          switch (state) {
            ...
            case 'pause':
              ...
              isProcessing = false;
              setBrowserButtonState(true);
              actualDownloadStarted = true; // ENHANCED: Mark actual download as started
              break;
            case 'resume':
      ...
      // ENHANCED: Validation with detailed limit explanations
      ...
      // ENHANCED: Improved download message with persistence
      ...
      const completeDataCleanup = async () => {
        try {
          // Clear persistent download message
          if (persistentDownloadMessage && persistentDownloadMessage.parentNode) {
            persistentDownloadMessage.parentNode.removeChild(persistentDownloadMessage);
            persistentDownloadMessage = null;
          }
        ...
        setProgressIndeterminate(true); // Reset to indeterminate
        hideMetrics();
      ...
      const showErrorMessage = (message, allowFallback = true) => {
        try {
          ...
          // Clear persistent download message on error
          if (persistentDownloadMessage && persistentDownloadMessage.parentNode) {
            persistentDownloadMessage.parentNode.removeChild(persistentDownloadMessage);
            persistentDownloadMessage = null;
          }
          ...
          // ENHANCED: Add detailed validation context
          if (isValidationError) {
          ...
          nodes.errorArea.innerHTML = `
            <div class="error-area">
              <div style="font-weight:500;margin-bottom:8px;">${safeMessage}</div>
              ${allowFallback ? `
      ...
      // ENHANCED: Improved blob download system
      const downloadBlobWithRealFilename = (blob, filename) => {
        return new Promise((resolve, reject) => {
          try {
            ...
            // Method 2: Enhanced blob download with better compatibility
            function downloadBlobMethod2() {
              try {
                ...
                // ENHANCED: Add data attributes for better browser compatibility
                ...
                try {
                  // ENHANCED: Multiple click methods for better compatibility
        ...
        try {
          ...
          downloadTask.on && downloadTask.on('start', (data = {}) => {
            try {
              ...
              // ENHANCED: Switch to determinate immediately when download starts
              setProgressIndeterminate(false);
          ...
          downloadTask.on && downloadTask.on('status', (data = {}) => {
            try {
              ...
              if (statusLower.includes('paused')) {
                ...
                stopIconPulse();
                pauseInProgress = false;
                // Keep determinate progress bar when paused
                ...
                setProgressIndeterminate(false); // Use determinate when resuming
          ...
          downloadTask.on && downloadTask.on('error', async (data = {}) => {
            try {
              ...
              showSettings();
              stopIconPulse();
              // Keep determinate progress bar to show current progress
      ...
      const startFallbackDownload = (reason, errorMessage) => {
        try {
          ...
          // ENHANCED: Use determinate progress for fallback
          setProgressIndeterminate(false);
      ...
      // ENHANCED: Improved action button handler with better state management
      const handleActionClick = async (event) => {
        try {
          ...
          // ENHANCED: Debouncing for pause clicks
              ...
              if (manager && typeof manager.resume === 'function') {
                ...
                if (resumeTask) {
                  ...
                  if (resumeTask.meta && resumeTask.meta.completedStarts) {
                    ...
                    actualDownloadStarted = true;
                    setProgressIndeterminate(false); // Use determinate for resume
      ...
      // ENHANCED: Start with indeterminate until actual download begins
      setProgressIndeterminate(true);
      showMetrics();
      if (nodes.metricsCard) nodes.metricsCard.innerHTML = 'Fetching details';
      ...
      try {
        if (manager && typeof manager.checkForExistingDownload === 'function') {
          ...
          if (existingDownload) {
            ...
            setProgressIndeterminate(false); // Show determinate with existing progress
            if (nodes.metricsCard) {
              nodes.metricsCard.innerHTML = 
